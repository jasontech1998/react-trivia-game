...
export function sendWelcomeMessages(
  webSocket: WebSocket,
  playerName: string,
...
export function notifyPlayersOfDeparture(
  game: Game,
  playerName: string,
  wss: WebSocket.Server,
  clients: Map<WebSocket, string>,
...
export function sendErrorAndClose(webSocket: WebSocket, type: string, message: string): void {
  webSocket.send(JSON.stringify({ type, message }));
  webSocket.close(1008, message);
}

export function notifyGameCreated(game: Game, wss: WebSocket.Server): void {
  broadcastToGame(
    game,
    {
      type: 'game_create',
...
export function notifyGameJoined(webSocket: WebSocket, game: Game): void {
  webSocket.send(
    JSON.stringify({
      type: 'game_joined',
      payload: {
