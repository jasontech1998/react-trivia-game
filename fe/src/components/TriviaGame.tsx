	...
	const handleMessage = (event: MessageEvent) => {
		...
		switch (data.type) {
			...
			case 'game_joined':
				...
				setGames(prevGames => prevGames.filter(g => g.id !== gameJoinedPayload.id));
				setError(null);
				break;
			case 'game_rejoined':
				setCurrentGame({
			...
			case 'player_joined':
				...
				setCurrentGame(prevGame => 
					prevGame && prevGame.id === data.payload.gameId 
						? { ...prevGame, playerCount: data.payload.playerCount, playerNames: data.payload.players }
						: prevGame
				);
			...
			case 'game_join_failed':
				setError(data.payload.message || 'Failed to join the game. Please try again.');
				break;
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-teal-100 to-cyan-100 p-4 flex items-center justify-center">
			<div className={`relative w-full max-w-4xl mx-auto transition-all duration-500 ease-in-out ${isInGame ? 'flex flex-col sm:flex-row' : 'block'}`}>
				...
				{/* Main card */}
				<div className={`bg-white shadow-lg rounded-3xl transition-all duration-500 ease-in-out ${isInGame ? 'w-full sm:w-2/3' : 'w-full'}`}>
								...
								<div className={isInGame ? '' : 'max-h-96 overflow-y-auto'}>
									{!currentGame && scores.length > 0 ? (
										<div className="bg-white rounded">
											<h2 className="text-2xl font-semibold mb-4 text-emerald-600">Game Over</h2>
											<h3 className="text-xl mb-2 text-cyan-500">Final Scores:</h3>
											<ul className="list-disc list-inside mb-4">
												{scores.sort((a, b) => b.score - a.score).map((score, index) => (
													<li key={index} className={`${score.name === playerName ? "font-bold" : ""} ${index === 0 ? "text-emerald-500" : "text-gray-700"}`}>
										...
										<div className="mb-6">
											<h2 className="text-2xl font-semibold mb-4 text-emerald-600">Create a New Game</h2>
										...
										<>
											{currentGame.state === 'waiting' && (
												<div className="text-center">
													<h2 className="text-2xl font-semibold mb-4 text-emerald-600">Waiting Room: {currentGame.name}</h2>
													<p className="text-xl mb-4 text-cyan-600">Number of Questions: {currentGame.questionCount}</p>
													{isLeader ? (
														<div className="space-y-2">
															...
															>
																{currentGame.playerNames.length < 2 ? 'Waiting for more players' : 'Start Game'}
															</button>
															<button 
											...
											{currentGame.state === 'countdown' && (
												<div className="text-center">
													<h2 className="text-2xl font-semibold mb-4 text-emerald-600">Game starting in...</h2>
													<p className="text-6xl font-bold text-cyan-500">{countdown}</p>
												</div>
											)}

											{currentGame.state === 'question' && currentQuestion && (
												<div className="bg-white rounded-lg shadow-md p-6">
													<h2 className="text-2xl font-semibold mb-4 text-emerald-600">{currentQuestion.question}</h2>
													<div className="flex justify-between items-center mb-4">
														<p className="text-xl text-cyan-500">Time left: {timeLeft} seconds</p>
														<div className="text-right">
															<p className="text-sm text-gray-500">Question {currentGame.currentQuestionIndex + 1} of {currentGame.questionCount}</p>
														</div>
													</div>
													{lastCorrectPlayer && (
					...
					<div className="bg-gradient-to-r from-purple-400 to-indigo-500 shadow-lg rounded-3xl mt-4 p-6">
						...
						<div className="space-y-2 max-h-64 overflow-y-auto">
							{connectedPlayers.map((player, index) => (
								<div key={index} className="flex items-center justify-between text-lg font-semibold bg-white bg-opacity-20 rounded-md px-3 py-1 shadow-sm">
									<span>{player}</span>
									{player === playerName && (
